...
// const s3MainDomain = "s3.amazonaws.com";
const s3MainDomain = "s3.bwsfs.de";

AWS.config.update({endpoint: `https://${s3MainDomain}`});
...
function object2hrefvirt(bucket, key, escape = false) {
    ...
    const rc = `${document.location.protocol}//${bucket}.${s3MainDomain}/${enckey}`;
    return escape ? htmlEscape(rc) : rc;
}
...
function object2hrefpath(bucket, key, escape = false) {
    const enckey = key.split('/').map(x => encodeURIComponent(x)).join('/');
    const rc = `${document.location.protocol}//${s3MainDomain}/${bucket}/${enckey}`;
    return escape ? htmlEscape(rc) : rc;
}
...
$(document).ready(() => {
    ...
    angular.element(document).injector().get("SharedService").changeSettings({
        auth: "anon",
        region: "",
        bucket: bucket,
        entered_bucket: bucket,
        selected_bucket: "",
        view: "folder",
        delimiter: "/",
        prefix: "",
